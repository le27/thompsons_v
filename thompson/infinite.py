...
class InfiniteAut(Automorphism):
	...
	def _split_basis(self):
		"""Partition X into type B and type C parts.r"""
	...
	def _congruence_graph(self):
		r"""Form a graph whose vertex set is the QNF basis. The edges store the information which makes two vertices congruent."""
	...
	def find_power_conjugators(self, other, cheat=False):
		r"""Tests two infinite factors to see if they are power conjugate. Yields minimal solutions :math:`(a, b, \rho)`.
	...
	def power_conjugacy_bounds(self, other):
		r"""Compute the bounds :math:`\hat a, \hat b` on powers which could solve the power conjugacy problem.
	...
	def compute_bounds(s_parts, o_parts):
		r"""Computes the bounds :math:`\hat a, \hat b` (in terms of the partitions :math:`P, Q` given by *s_parts* and *o_parts*) as in eqns (14) and (15) of the paper.
